    ...
    def _should_skip(self, frame):
        ...
        while f:
            if fname.startswith("/usr/lib/"):
                return True
            if "addons/script.module.ptwfanvod/lib/ptw/debug" in fname:
                return True
            f = f.f_back
        return False
            ...
            try:
                ...
                if isinstance(v, list):
                    if v and isinstance(v[0], basestring):
                        return list(
                            s[:100] if isinstance(s, basestring) else s for s in v[:50]
                        )
                    return v[:50]
                return v
            except:
                xbmc.log(
                    '{pre}Getting varaible "{var}" failed.'.format(pre=pre, var=k),
                    xbmc.LOGDEBUG,
                )
                return "???!!"
